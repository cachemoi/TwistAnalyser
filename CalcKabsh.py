...
"""
from PullPDB3D1 import proteins
import numpy as np
import re
...
2) Kabsch: The RMSD after the two coordinate sets are translated and rotated onto each other.
    ...
    parser.add_argument('-f', '--format', action='store', help='Format of input files', default="xyz")

    args = parser.parse_args()
    atomsP, P = get_coordinates_pdb(args.structure_a, True)
    atomsQ, Q = get_coordinates_pdb(args.structure_b, True)
"""
    # Calculate 'dumb' RMSD

    atomsP = proteins.atoms['1cag']
    atomsQ = proteins.atoms['1cgd']

    P = proteins.coord['1cag']
    Q = proteins.coord['1cgd']
