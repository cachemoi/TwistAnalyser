# version to deal with total P and register independance
...
class MetaData:
    def __init__(self):
        ...
        self.species_total_P = {}
        self.species_total_P_percentage = {}
...
def CalcMetaData():
    ...
    for code in meta_data.all_angles_averages:
        angle_array.append(meta_data.all_angles_averages[code])

    for code in meta_data.all_translation_averages:
        trans_array.append(meta_data.all_translation_averages[code])

    for angle, translation in zip(angle_array, trans_array):
        sum_angles += angle
        sum_translation += translation

    meta_data.all_average_angles = sum_angles / len(angle_array)
    meta_data.all_average_translation = sum_translation / len(trans_array)
...
def AddToMeta(code):
    ...
    meta_data.all_angles_averages[code] = total.average_angle
    meta_data.all_translation_averages[code] = total.average_translation

    meta_data.all_P[code] = total.P_number
    meta_data.all_P_percentage[code] = total.P_number*100/total.all_count
...
def PlotPGraph():
    ...
    for code in meta_data.all_P_percentage:
        P_percentage = meta_data.all_P_percentage[code]
        angle = meta_data.all_angles_averages[code]

        barx14.append(float("{0:.2f}".format(round(P_percentage, 2))))
        bary14.append(float("{0:.2f}".format(round(angle, 2))))

    barx14_min = min(barx14)
    barx14_max = max(barx14)

    bary14_min = min(bary14)
    bary14_max = max(bary14)

    print(bary14_min)
    print(bary14_max)

    fig, = plt.plot(barx14, bary14, 'ro')
    plt.xlim(barx14_min, barx14_max)
    plt.ylim(-106.6, -105.8)
    fig.axes.get_yaxis().get_major_formatter().set_useOffset(False)
    plt.ylabel('helical twist (K°)')
    plt.xlabel("P percentage")
    plt.title('Dependence of internal triple helical twist on P content')

    plt.tight_layout()

    file_open.SetFile("Ppercents", "angle P relationship.png")
    plt.savefig(file_open.file_path)
    ...
    for code in meta_data.all_P_percentage:
        P_percentage = meta_data.all_P_percentage[code]
        t = meta_data.all_translation_averages[code]

        barx15.append(float("{0:.2f}".format(round(P_percentage, 2))))
        bary15.append(float("{0:.2f}".format(round(t, 2))))

    barx15_min = min(barx15)
    barx15_max = max(barx15)

    print(barx15)
    print(bary15)

    plt.plot(barx15, bary15, 'ro')
    plt.ylim(2.70, 3)
    plt.xlim(barx15_min, barx15_max)
    plt.axis('auto')
    plt.ylabel('helical translation (Å)')
    plt.xlabel("P percentage")
    plt.title('Dependence of internal triple helical translation on P content')

    plt.tight_layout()

    file_open.SetFile("Ppercents", "translation P relationship.png")
    plt.savefig(file_open.file_path)
...
def PlotMetaData():
    ...
    for translation in trans_array:
        formatted.append(float("{0:.2f}".format(round(translation,2))))
...
def SaveMetaData():
    ...
    for specie in meta_data.species_steps_percentage:
        ...
        file_specie.SetFile("Metadata",specie)
...
def main(UP=False, PDB=False, UI="", GXY_num=None, SP=False,OV=False, inc=False, ND=None, DOM=False):
    ...
    if UP is True:
        ...
        print(meta_data.all_aa_count_percentage)
...
#main(PDB=True, DOM = True, inc=True, OV=True, SP=True)
